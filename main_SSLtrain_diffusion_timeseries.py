...
from train.train_diffusion_timeseries import run_training
import matplotlib
    ...
    for params_values in it.product(  *dataset_params.values() ):
       ...
       # print(dataset_param)
        ...
        for train_param,net_param,loss_param,optimizer_param in parameters_list:
            ...
            net_param["dataset_nf"] = dataset[0].shape[1]

            net_param["pretrain_f_path"]="results/pre_model_F"+new_model_file_relatvie_path
            net_param["pretrain_g_path"]="results/pre_model_g"+new_model_file_relatvie_path
...
def parallel_grid_search(dataset_params,train_params,net_params,loss_params,optimizer_params,records_path):
   ...
   # configs_record_scores=list()
    ...
    for params_values in it.product(  *dataset_params.values() ):
        dataset_param=dict(zip(dataset_params.keys(),params_values))
      #  print(dataset_param)
        dataset = pre_DataSet_Timeseries(file_path=dataset_param["dataset_path"], features=dataset_param["dataset_features"],
