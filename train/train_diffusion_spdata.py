...
from utils.utils import save_checkpoint, emergency_checkpoint, load_emergency_checkpoint
...
def run_training( trainset, validationset,train_param,net_param,loss_param,optimizer_param,records_path):
  ...
  #  validationset_data = torch.cat(validationset["data"], dim=0)  # n*pred_len,F
    ...
    #设置模型性能评价指标参数
    current_step = load_emergency_checkpoint(checkpoint_path=records_path,
                                             model=train_GNN,
                                             optimizer=optimizer,
        ...
        for epoch in range(init_epoch,train_param["train_epochs"]):
            ...
            for n,data in enumerate(tqdm(train_loader, desc=f"Epoch {epoch}")):
               ...
               # print("\r******************train_epoch:{} batch:{}********************\n".format(epoch,n),end=" ")
                ...
                if train_GNN.scaler == "StandardScaler":
                    data.x = train_GNN.scaler_transform(data.x)
               ...
               # time_step=time.time()
                #out: [N,2]
                if train_param.get("train_model_select") is None or train_param["train_model_select"]=="NsDiff_model":
                    loss = train_GNN.training_step(gdata=data)  # diffusion training
                elif train_param["train_model_select"]=="pretrain_f":
                    loss = train_GNN.pretrain_f(gdata=data)  # pretrain f
                elif train_param["train_model_select"]=="pretrain_g":
                    loss = train_GNN.pretrain_g(gdata=data)  # pretrain g
                else:
                    raise ValueError("train_model_select error")
            ...
            if epoch%train_param["ckpt_period"]==0 and epoch!=0 and train_param["ckpt"]:
                ...
                if os.path.exists(sckpt_path):
                    ...
                    os.mkdir(sckpt_path)
                if train_param.get("train_model_select") is None or train_param["train_model_select"]=="NsDiff_model":
                    save_checkpoint(path=sckpt_path,model_name="tmpt_model_{}iter".format(epoch),model=train_GNN,net_param=net_param)
                elif train_param["train_model_select"]=="pretrain_f":
                    save_checkpoint(path=sckpt_path,model_name="tmpt_model_{}iter".format(epoch),model=train_GNN.cond_pred_model,net_param=net_param)
                elif train_param["train_model_select"]=="pretrain_g":
                    save_checkpoint(path=sckpt_path,model_name="tmpt_model_{}iter".format(epoch),model=train_GNN.cond_pred_model_g,net_param=net_param)
                else:
                    raise ValueError("train_model_select error")
