...
def run_training( trainset, validationset,train_param,net_param,loss_param,optimizer_param,records_path):
    ...
    # 加载数据集
    trainset_data = torch.cat([data.x for data in trainset], dim=0)  # n*num_nodes,pred_len,F
  #  validationset_data = torch.cat(validationset["data"], dim=0)  # n*pred_len,F

    # train_sampler=DistributedSampler(trainset,  shuffle=True, drop_last=False)
    # val_sampler=DistributedSampler(validationset,  shuffle=True, drop_last=False)

    # train_loader = DataLoader(trainset, sampler=train_sampler,batch_size=train_param["train_batch_size"], )
    # val_loader   = DataLoader(validationset,sampler=val_sampler, batch_size=train_param["val_batch_size"], )

    train_loader = DataListLoader(trainset, batch_size=train_param["train_batch_size"], shuffle=True, drop_last=False)
    val_loader = DataListLoader(validationset, batch_size=train_param["val_batch_size"], shuffle=False, drop_last=False)

    #设置训练模型参数、优化器参数、损失函数参数

    train_GNN = diffusion_models(task_model=net_param["task_model"],net_param=net_param,
                                 train_model_select=train_param.get("train_model_select"),
                                 pretrain_f_path=net_param["pretrain_f_path"] if net_param.get("pretrain_f_path") is not None else None,
                                 pretrain_g_path=net_param["pretrain_g_path"] if net_param.get("pretrain_g_path") is not None else None,
                                 ).to(net_param["device"])

    if train_GNN.scaler == "StandardScaler":
        train_GNN.scaler_fit(trainset_data)
    train_GNN=torch.nn.DataParallel(train_GNN).to(net_param["device"])
    ...
    try:
        for epoch in range(init_epoch,train_param["train_epochs"]):
            ...
            #批量训练
            for n,data_list in enumerate(tqdm(train_loader, desc=f"Epoch {epoch}")):
               ...
               # time_step=time.time()
                #out: [N,2]
                loss = train_GNN(gdatalist=data_list)
            ...
            with torch.no_grad():
                if  train_param["test_set"]:
                    ...
                    train_GNN.eval()
                    for n, data_list in enumerate(val_loader):
                        loss = train_GNN(gdatalist=data_list)
            ...
            if epoch%train_param["ckpt_period"]==0 and epoch!=0 and train_param["ckpt"]:
                ...
                if train_param.get("train_model_select") is None or train_param["train_model_select"]=="NsDiff_model":
                    save_checkpoint(path=sckpt_path,model_name="tmpt_model_{}iter".format(epoch),model=train_GNN.module,net_param=net_param)
                elif train_param["train_model_select"]=="pretrain_f":
                    save_checkpoint(path=sckpt_path,model_name="tmpt_model_{}iter".format(epoch),model=train_GNN.module.cond_pred_model,net_param=net_param)
                elif train_param["train_model_select"]=="pretrain_g":
                    save_checkpoint(path=sckpt_path,model_name="tmpt_model_{}iter".format(epoch),model=train_GNN.module.cond_pred_model_g,net_param=net_param)
                else:
                    raise ValueError("train_model_select error")
    ...
    if train_param.get("train_model_select") is None or train_param["train_model_select"]=="NsDiff_model":
        ...
        save_checkpoint(path=model_path, model_name="model_trained", model=train_GNN.module.cond_pred_model,
                    net_param=net_param)
    elif train_param["train_model_select"]=="pretrain_g":
        save_checkpoint(path=model_path, model_name="model_trained", model=train_GNN.module.cond_pred_model_g,
                    net_param=net_param)
    else:
        raise ValueError("train_model_select error")
    #保存训练模型性能指标数据 文件夹可略去
