...
class SISDynamicsSDE(torch.nn.Module):
    ...
    def f(self, t, x):
        f = -self.d * x
        f += self.epsilon * torch.mul(1 - x, torch.mm(self.A, x))
        return f
...
def simulate_with_burn_in(graph_data,base_params, file_save_path=None,total_time=100.0, burn_time=20, dt=0.1,max_one_time=1e6):
    ...
    # 第一阶段：预热期 (固定N=0.0)
    sde_burn = SISDynamicsSDE(**base_params,graph_data=graph_data).to(device)  # 预热期固定N=0.0
    ...
    # 第二阶段：动态变化期 (N随时间变化)
    sde_dynamic = SISDynamicsSDE( **base_params, graph_data=graph_data).to(device)  # 动态变化期N随时间变化
    y0_dynamic = ys_burn[-1].reshape(-1, 1).to(device)  #   # 从预热结束状态开始
    dy_data_record={}
    if  file_save_path is not None and os.path.exists(file_save_path+"/"+'SIS_dynamic_eta{}d{}epsilon{}'.format(sde_dynamic.eta,sde_dynamic.d,sde_dynamic.epsilon)  + '.pt'):
        dy_data_record = torch.load(file_save_path+"/"+'SIS_dynamic_eta{}d{}epsilon{}'.format(sde_dynamic.eta,sde_dynamic.d,sde_dynamic.epsilon) + '.pt')
        ...
        if max_one_time>=total_time:
            ...
            if file_save_path is not None:
                torch.save(dy_data_record, file_save_path+"/" + 'SIS_dynamic_eta{}d{}epsilon{}'.format(sde_dynamic.eta,sde_dynamic.d,sde_dynamic.epsilon)  + '.pt')
       ...
       # print("lambda_max",lambda_max)
        ...
        for base_params in grid_dy_params(base_params_dict):
          #  start_time = time.time()
            count=0
            while True:
                ts, ys, tp_values = simulate_with_burn_in(graph_data=graph_data, base_params=base_params,  total_time=total_time,
                                                         burn_time=burn_time,file_save_path=graph_file_path,max_one_time=1000,dt=0.1)
                count+=1
                ...
                if count>10:
                    print(graph_name)
                    print('SIS_dynamic_eta{}d{}epsilon{}'.format(base_params['eta'],base_params['d'],base_params["epsilon"] ))
