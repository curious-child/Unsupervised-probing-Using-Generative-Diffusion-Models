...
class EcosystemSDE(torch.nn.Module):
    ...
    def f(self, t, x):
        ...
        harvesting = -self.epsilon * (x**2) / (x**2 + 1)
...
def simulate_with_burn_in(graph_data,base_params, file_save_path=None,total_time=100.0, burn_time=20, dt=0.1,max_one_time=1e6):
    ...
    """
    # 第一阶段：预热期 (固定N=0.0)
    sde_burn = EcosystemSDE(**base_params,graph_data=graph_data).to(device)  # 预热期固定N=0.0
    ...
    # 第二阶段：动态变化期 (N随时间变化)
    sde_dynamic = EcosystemSDE( **base_params, graph_data=graph_data).to(device)  # 动态变化期N随时间变化
    y0_dynamic = ys_burn[-1].reshape(-1, 1).to(device)  #   # 从预热结束状态开始
    dy_data_record={}
    if  file_save_path is not None and os.path.exists(file_save_path+"/"+'biomass_dynamic_eta{}r{}epsilon{}'.format(sde_dynamic.eta,sde_dynamic.r,sde_dynamic.epsilon) + '.pt'):
        dy_data_record = torch.load(file_save_path+"/"+'biomass_dynamic_eta{}r{}epsilon{}'.format(sde_dynamic.eta,sde_dynamic.r,sde_dynamic.epsilon) + '.pt')
        ...
        if max_one_time>=total_time:
            ...
            if file_save_path is not None:
                torch.save(dy_data_record, file_save_path+"/" + 'biomass_dynamic_eta{}r{}epsilon{}'.format(sde_dynamic.eta,sde_dynamic.r,sde_dynamic.epsilon) + '.pt')
...
if __name__ == '__main__':
    ...
    file_save_path="spdata_sde_biomass_contant"
   # file_save_path=None
    base_params_dict = {
        'r': [0.7,0.8,0.9],  # 内禀增长率
        'k': [10.0],  # 承载能力
        ...
        for base_params in grid_dy_params(base_params_dict):
          #  start_time = time.time()
              ...
              while True:
                  ...
                  if count > 10:
                      print(graph_name)
                      print('biomass_dynamic_eta{}r{}espilon{}'.format(base_params['eta'],base_params['r'],base_params['espilon']))
