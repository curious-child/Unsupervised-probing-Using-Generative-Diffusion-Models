...
class NeuronalDynamicsSDE(torch.nn.Module):
    # 严格遵循图片中的公式
    noise_type = 'diagonal'  # 每个节点独立噪声 (W_i)
    sde_type = 'ito'  # Ito 随机积分

    def __init__(self, epsilon, tau, mu, eta, graph_data):
        """
    ...
    def f(self, t, x):
        ...
        sigmoid_term = torch.sigmoid(self.tau * (x - self.mu))
        # 使用稀疏矩阵乘法计算连接强度
        interaction = (1 - self.epsilon) * torch.mm(self.A, sigmoid_term)
...
def simulate_with_burn_in(graph_data,base_params, file_save_path=None,total_time=100.0, burn_time=20, dt=0.1,max_one_time=1e6):
    """
    带预热期的动态模拟
    total_time: 总模拟时间
    burn_time: 预热期时间
    ...
    """
    # 第一阶段：预热期 (固定N=0.0)
    sde_burn = NeuronalDynamicsSDE(**base_params,graph_data=graph_data).to(device)  # 预热期固定N=0.0
    ...
    # 第二阶段：动态变化期 (N随时间变化)
    sde_dynamic = NeuronalDynamicsSDE(**base_params, graph_data=graph_data).to(device)  # 动态变化期N随时间变化
    y0_dynamic = ys_burn[-1].reshape(-1, 1).to(device)  #   # 从预热结束状态开始
    dy_data_record={}
    if  file_save_path is not None and os.path.exists(file_save_path+"/"+'neuronal_dynamic_eta{}tau{}epsilon{}'.format(sde_dynamic.eta,sde_dynamic.tau,sde_dynamic.epsilon) + '.pt'):
        dy_data_record = torch.load(file_save_path+"/"+'neuronal_dynamic_eta{}tau{}epsilon{}'.format(sde_dynamic.eta,sde_dynamic.tau,sde_dynamic.epsilon) + '.pt')
        ...
        if max_one_time>=total_time:
            ...
            with torch.no_grad():
                ...
                # 计算每个时间点的N值
            if torch.max(ys_dynamic.mean(dim=1)) > 1e3 or torch.min(
                    ys_dynamic.mean(dim=1)) < -5 or torch.isnan(ys_dynamic.mean(dim=1)).any():
                #  print("Warning: the mean of ys_dynamic is out of range [-5,100]")

                return None, None, None
            tp_values = torch.tensor([base_params["epsilon"] for t in ts_dynamic])
            ...
            if file_save_path is not None:
                torch.save(dy_data_record, file_save_path+"/" + 'neuronal_dynamic_eta{}tau{}epsilon{}'.format(sde_dynamic.eta,sde_dynamic.tau,sde_dynamic.epsilon) + '.pt')
            ...
            for i in range(int(total_time//max_one_time)):
              #  print("i",i)
                ...
                temp_tp_values = torch.tensor([base_params["epsilon"] for t in temp_ts_dynamic])
                if torch.max(temp_ys_dynamic.mean(dim=1)) > 1e3 or torch.min(
                        temp_ys_dynamic.mean(dim=1)) < -5 or torch.isnan(temp_ys_dynamic.mean(dim=1)).any():
                  #  print("Warning: the mean of ys_dynamic is out of range [-5,100]")
...
if __name__ == '__main__':
    ...
    file_save_path="spdata_sde_neuronal_contant"
   # file_save_path=None
    base_params_dict = {
        'tau': [2.0,2.2,2.5],  #2.0,2.5
        'mu': [1.5 ],  #
        'eta': [1e-2,1e-4],  # 噪声强度,0.05,0.005
        "epsilon":[0.2,0.4,0.8]
    }
    burn_time = 100  # 预热期时长
    ...
    for file in tqdm(sorted(glob(graph_file_path+'/'+"*"+".graphml"))):
        ...
        for base_params in grid_dy_params(base_params_dict):
          #  start_time = time.time()
           count = 0
           while True:
              ts, ys, tp_values = simulate_with_burn_in(graph_data=graph_data,
                                                        base_params=base_params,
                                                        total_time=total_time,
                                                        burn_time=burn_time, file_save_path=graph_file_path,
                                                        max_one_time=1000, dt=0.1, )
              count += 1
              ...
              if count > 10:
                  print(graph_name)
                  print('neuronal_dynamic_eta{}tau{}epsilon{}'.format(base_params['eta'],base_params['tau'],base_params["epsilon"]))
                  print("Warning: the simulation failed")
                  break
           # print("运行时间", time.time() - start_time)
            # 可视化结果

           # plot_dynamic_trajectory(ts, ys, tp_values, file_save_path=None,save_name='neuronal_dynamic_eta{}tau{}_{}'.format(base_params['eta'],base_params['tau'],trend))