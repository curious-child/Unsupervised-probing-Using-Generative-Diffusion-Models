...
class NeuronalDynamicsSDE(torch.nn.Module):
    ...
    def f(self, t, x):
        ...
        # 使用稀疏矩阵乘法计算连接强度
        interaction = (1 - self.epsilon(t.item())) * torch.mm(self.A, sigmoid_term)
...
def simulate_with_burn_in(graph_data,epsilon_func,base_params,trend,epsilon_init=0.0, file_save_path=None,total_time=100.0, burn_time=20, dt=0.1,max_one_time=1e6):
    ...
    """
    # 第一阶段：预热期 (固定N=0.0)
    sde_burn = NeuronalDynamicsSDE(epsilon=lambda t: epsilon_init,**base_params,graph_data=graph_data).to(device)  # 预热期固定N=0.0
    ...
    # 第二阶段：动态变化期 (N随时间变化)
    sde_dynamic = NeuronalDynamicsSDE(epsilon=epsilon_func, **base_params, graph_data=graph_data).to(device)  # 动态变化期N随时间变化
    y0_dynamic = ys_burn[-1].reshape(-1, 1).to(device)  #   # 从预热结束状态开始
    dy_data_record={}
    if  file_save_path is not None and os.path.exists(file_save_path+"/"+'neuronal_dynamic_eta{}tau{}_{}'.format(sde_dynamic.eta,sde_dynamic.tau,trend) + '.pt'):
        dy_data_record = torch.load(file_save_path+"/"+'neuronal_dynamic_eta{}tau{}_{}'.format(sde_dynamic.eta,sde_dynamic.tau,trend) + '.pt')
        ...
        if max_one_time>=total_time:
            ...
            if file_save_path is not None:
                torch.save(dy_data_record, file_save_path+"/" + 'neuronal_dynamic_eta{}tau{}_{}'.format(sde_dynamic.eta,sde_dynamic.tau,trend) + '.pt')
...
if __name__ == '__main__':
    ...
    file_save_path="spdata_sde_neuronal"
   # file_save_path=None
    base_params_dict = {
            ...
            for base_params in grid_dy_params(base_params_dict):
              #  start_time = time.time()
               ...
               while True:
                  ...
                  if count > 10:
                      print(graph_name)
                      print('neuronal_dynamic_eta{}tau{}_{}'.format(base_params['eta'],base_params['tau'],trend))
