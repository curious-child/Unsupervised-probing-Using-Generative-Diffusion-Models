...
def plot_ews_compare(plt_data_dict,sample_ts,dynamic_type,sample_window_step):
    ...
    axs[0].plot(ts, ys.mean(axis=1), 'b-', linewidth=2)
   ...
   # y1=np.array(plt_data_dict["pred_mean"])
    ...
    axs[1].plot(plt_data_dict["biomass"]["ews_ts"][:len(plt_data_dict["biomass"]["ews"])], plt_data_dict["biomass"]["ews"], 'r.', linewidth=2)
    axs[1].set_ylabel('Predicted Uncertainty \n(trained on biomass)')
    axs[1].sharex(axs[0])

    axs[2].plot(plt_data_dict["neuronal"]["ews_ts"][:len(plt_data_dict["biomass"]["ews"])], plt_data_dict["neuronal"]["ews"], 'r.', linewidth=2)
    axs[2].set_ylabel('Predicted Uncertainty \n(trained on neuronal)')
    axs[2].sharex(axs[0])

    axs[3].plot(plt_data_dict["SIS"]["ews_ts"][:len(plt_data_dict["SIS"]["ews"])], plt_data_dict["SIS"]["ews"], 'r.', linewidth=2)
    axs[3].set_ylabel('Predicted Uncertainty \n(trained on SIS)')
    axs[3].sharex(axs[0])
...
if __name__ == '__main__':
    ...
    dataset_types = ["biomass","neuronal","SIS"]

    model_types = ["biomass","neuronal","SIS"]
    ...
    for data_trend in data_trends:
        ...
        for dataset_type in dataset_types:
            ...
            for model_type in model_types:
                plt_data_dict[model_type]={}
                print("*" * 10 + "graph_type:{}".format(model_type) + "*" * 10 + "\n")
                model_save_file = "ews_results/ews_generalization/dynamic/{}".format(model_type)
                spdata_file = "dataset/spdata_sde_{}/barabasi_albert_30_0/".format(dataset_type)+spdata_name
                ews_file_path=model_save_file+"/{}".format(spdata_name)
                print(ews_file_path+"\n")
                ...
                plt_data_dict[model_type]["pred_mean"] = pred_mean
                plt_data_dict[model_type]["ews"]=uncertainty_ews_list
                plt_data_dict[model_type]["ews_ts"]=sample_timepoints
            ...
            fig.savefig("ews_results/ews_generalization/dynamic"+"/{}_{}.svg".format(dataset_type,data_trend))
          #  plt.show()