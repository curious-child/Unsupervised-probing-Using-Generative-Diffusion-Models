...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x, t, edge_index):
       ...
       # print("before downsampling h:",h.shape)
...
class UGnet(nn.Module):
    def __init__(self, net_param) -> None:
        ...
        self.T_p = net_param.pred_len
        self.T_h = net_param.windows
        self.T = self.T_p
        self.F = net_param.dataset_nf
       ...
       # print("T_in:",T_in)
        ...
        assert T_in == self.T, "T_in should be equal to T"
        self.x_proj = nn.Conv2d(3*self.F, self.d_h, (1,1))
        self.out = nn.Sequential(nn.Conv2d(self.d_h, self.d_h, (1,1)),
                                 nn.Linear( self.T, self.T),)
        self.lin4 = nn.Linear(self.d_h, self.F)
        self.sigma_lin = nn.Linear(self.d_h, self.F)
        # for gcn



    def forward(self, y_t, y_0_hat, g_x, t,edge_index):
        """

        :return:
        """
        x = torch.cat((y_t, y_0_hat, g_x), dim=-1)  # (B*V, T_p, 3*F)
        x = x.unsqueeze(2).transpose(1, 3)  # (B*V, 3*F, 1,T_p)


        x = self.x_proj(x)# (B*V, d_h,1,  T_p)
        ...
        for m in self.up:
            if isinstance(m,  Upsample):
                ...
                x = m(x,t, edge_index)
      #  print(" out x:",x.shape)
        e = self.out(x)#(B*V, F,1,T_p)
        eps_pred = e.squeeze(2).transpose(1, 2) # (B*V, T,F_p)
