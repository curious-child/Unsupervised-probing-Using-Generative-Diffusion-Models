...
def cal_sigma12(alphas,alphas_cumprod,alphas_cumprod_sum, alpha_bar_prev, alphas_cumprod_sum_prev, betas_tiled_m_1, betas_bar_m_1, gx, y_sigma, t):
    ...
    at = extract(alphas, t, gx).to(gx.device)
    at_bar = extract(alphas_cumprod, t, gx).to(gx.device)
    # at_bar_prev = extract(alpha_bar_prev, t, gx)
    at_tilde = extract(alphas_cumprod_sum, t, gx).to(gx.device)
    b_tilde_m_1 = extract(betas_tiled_m_1, t, gx).to(gx.device)
    ...
    return at, at_bar.to(gx.device), at_tilde.to(gx.device), Sigma_1.to(gx.device), Sigma_2.to(gx.device)

def cal_forward_noise(betas_tiled, betas_bar, gx, y_sigma, t):
    b_bar_t =  extract(betas_bar, t, gx).to(gx.device)
    b_tilded_t =  extract(betas_tiled, t, gx).to(gx.device)
...
def p_sample(model,  y, y_0_hat, gx, y_T_mean, t, alphas, one_minus_alphas_bar_sqrt, alphas_cumprod,alphas_cumprod_sum, alpha_bar_prev, alphas_cumprod_sum_prev, betas_tiled_all, betas_bar_all, betas_tiled_m_1_all, betas_bar_m_1_all):
    """
    ...
    eps_theta, sigma_theta = model( y, y_0_hat, gx, t)
...
def p_sample_t_1to0(model, y, y_0_hat, gx, y_T_mean, one_minus_alphas_bar_sqrt,alphas,alphas_cumprod,alphas_cumprod_sum,alpha_bar_prev, alphas_cumprod_sum_prev,betas_tiled_all, betas_bar_all, betas_tiled_m_1_all, betas_bar_m_1_all):
    device = next(model.parameters()).device
...
def p_sample_loop(model,  y_0_hat, gx, y_T_mean, n_steps, alphas, one_minus_alphas_bar_sqrt, alphas_cumprod,alphas_cumprod_sum, alpha_bar_prev, alphas_cumprod_sum_prev, betas_tiled, betas_bar, betas_tiled_m_1, betas_bar_m_1):
    device = next(model.parameters()).device
    ...
    for t in reversed(range(1, n_steps)):  # t from T to 2
        y_t = cur_y
        cur_y = p_sample(model, y_t, y_0_hat, gx, y_T_mean, t, alphas, one_minus_alphas_bar_sqrt,alphas_cumprod,alphas_cumprod_sum, alpha_bar_prev, alphas_cumprod_sum_prev, betas_tiled, betas_bar, betas_tiled_m_1, betas_bar_m_1)  # y_{t-1}
        y_p_seq.append(cur_y)
    assert len(y_p_seq) == n_steps
    y_0 = p_sample_t_1to0(model, y_p_seq[-1], y_0_hat, gx, y_T_mean, one_minus_alphas_bar_sqrt,alphas,alphas_cumprod,alphas_cumprod_sum, alpha_bar_prev, alphas_cumprod_sum_prev, betas_tiled, betas_bar, betas_tiled_m_1, betas_bar_m_1)
    y_p_seq.append(y_0)
    return y_p_seq
