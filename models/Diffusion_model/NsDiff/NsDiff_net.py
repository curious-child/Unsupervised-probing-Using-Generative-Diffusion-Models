...
import yaml
import argparse
from .tmdm_diffusion_utils import *
...
class NsDiff_net(nn.Module):
    """
    Vanilla Transformer
    """

    def __init__(self, configs, device):
        super(NsDiff_net, self).__init__()
        ...
        self.num_timesteps = configs.diffusion_steps
        self.dataset_object = None
        betas = make_beta_schedule(schedule=configs.diffusion_schedule, num_timesteps=configs.diffusion_steps,
        ...
        if configs.diffusion_schedule == "cosine":
    ...
    def forward(self, y_t, y_0_hat, gx, t):
        # print("y-t",y_t.shape)
        # print("t",t.shape)
        if t.shape[0] != y_t.shape[0]:
            t=t.repeat(y_t.shape[0])
        dec_out, sigma = self.diffussion_model( y_t, y_0_hat, gx, t)
        # print(self.diffussion_model.device_ids)
        # print(self.diffussion_model.output_device)
        # print("dec_out",dec_out.shape)
        return dec_out, sigma


class NsDiff_net_spatial(nn.Module):
    """
    Vanilla Transformer with spatial embedding
    """

    def __init__(self, configs, device):
        super(NsDiff_net_spatial, self).__init__()
        ...
        # CATE MLP
        self.diffussion_model = UGnet(configs)
        # if torch.cuda.device_count() > 1:
        #     self.diffussion_model = nn.DataParallel(self.diffussion_model)
        self.edge_index = None
    def set_edge_index(self,edge_index):
        self.edge_index = edge_index
    def forward(self, y_t, y_0_hat, gx, t,edge_index=None):
        if edge_index is None:
            edge_index = self.edge_index
            assert edge_index is not None, "edge_index must be set before forward pass"
        dec_out, sigma = self.diffussion_model(y_t, y_0_hat, gx, t,edge_index)
