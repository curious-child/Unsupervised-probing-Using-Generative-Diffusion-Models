...
def wv_sigma_trailing(x_enc, window_size, discard_rep=False):
    ...
    if not discard_rep:
        x_enc = F.pad(x_enc, (0, 0, window_size-1, 0), mode='replicate')  # Shape: (B, T + window_size-1, N)
    ...
    windows = x_enc.unfold(1, window_size, 1)#shape: (B, T, N, window_size)
    windows = windows.permute(0, 1, 3, 2)  # Shape: (B, T, window_size, N)

    # Compute variance across the window dimension (dim=2)
    sigma = windows.var(dim=2, unbiased=False)  # Shape: (B, T, N)
    return sigma
